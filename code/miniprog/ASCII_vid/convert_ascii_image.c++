...
string pTA (int strength_display) {
...
found_video:
    ...
    int t_width = 250;         
    int o_width = cap.get (CAP_PROP_FRAME_WIDTH);
    int o_height = cap.get (CAP_PROP_FRAME_HEIGHT);
    int t_height = static_cast<int> ((t_width * o_height / (double) o_width) * 0.45);
    cout << "Original res: " << o_width << "x" << o_height << endl;
    cout << "ASCII res: " << t_width << "x" << t_height << endl;
    ...
    while (true) {
        ...
        resize (gray_frame, resized_frame, Size (t_width, t_height), 0, 0, INTER_LINEAR);
        string outln;
